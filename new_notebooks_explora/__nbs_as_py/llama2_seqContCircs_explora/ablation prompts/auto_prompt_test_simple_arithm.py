...
def gen_addition_prompts(num_prompts):
    sequences = []
    next_members = []
...
def gen_subtraction_prompts(num_prompts):
    sequences = []
    next_members = []
...
def gen_multiplication_prompts(num_prompts):
    sequences = []
    next_members = []

    single_digit_multiplications = [(random.randint(0, 9), random.randint(0, 9)) for _ in range(num_prompts)]
    double_digit_multiplications = [(random.randint(10, 99), random.randint(10, 99)) for _ in range(num_prompts)]

    for a, b in single_digit_multiplications:
        prompt = f"{a} * {b} = "
        answer = str(a * b)
        sequences.append(prompt)
        next_members.append(answer)

    for a, b in double_digit_multiplications:
        prompt = f"{a} * {b} = "
        answer = str(a * b)
        sequences.append(prompt)
        next_members.append(answer)
...
sequences, next_members = gen_multiplication_prompts(5)
...
clean_gen(model, sequences_as_str[-1], next_members[-1])
...
num_prompts = 5
sequences_as_str, next_members = gen_subtraction_prompts(num_prompts)
...
num_prompts = 5
sequences_as_str, next_members = gen_multiplication_prompts(num_prompts)

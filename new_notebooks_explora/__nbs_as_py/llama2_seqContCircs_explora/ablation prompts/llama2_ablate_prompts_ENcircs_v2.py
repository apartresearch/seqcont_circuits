...
with open('template_1_unablated.txt', 'w') as f:
    for line in outputs:
        f.write(f"{line}\n")
files.download('template_1_unablated.txt')
...
def ablate_randcirc_autoScore_2(model, sequences_as_str, lst_rand_head_to_remove): # next_members,
    ...
    # perc_score = sum(all_scores) / len(next_members)
    # return perc_score #, list_outputs
    return all_prompt_outputs


# In[ ]:


randAbl_prompt_outputs = ablate_randcirc_autoScore_2(model, correct_prompts[0:2], lst_rand_head_to_remove[0:10])
randAbl_prompt_outputs


# In[ ]:


with open('template_1_rand_prompt1.txt', 'w') as f:
    for line in randAbl_prompt_outputs:
        f.write(f"{line}\n")
files.download('template_1_rand_prompt1.txt')
...
# intersect_all , show first two results
...
with open('template_1_big4.txt', 'w') as f:
    for line in outputs:
        f.write(f"{line}\n")
files.download('template_1_big4.txt')


# In[ ]:


# random, len 4 (not from saved head combo presets) ; show first two results
...
# ### debug why some ans are all 0% correct for rand

# likely because they weren't correct even when unablated. we find they aren't; but why were they above? start anew in v3 to save these output reulsts.
...
clean_text = 'Be concise. If today is April 19th, then in 25 days it will be'
ablate_then_gen(model, clean_text, corr_text, heads_not_ablate, mlps_not_ablate, 5)
...
clean_text = 'If today is April 19th, then in 25 days it will be'
clean_text = instruction + clean_text
corr_text = "uno uno uno" # dos tres cinco seis
heads_not_ablate = [(layer, head) for layer in range(32) for head in range(32)]  # unablated
mlps_not_ablate = [layer for layer in range(32)]
...
# # If today is November 20th, then in 28 days it will be

# In[ ]:


clean_text = "Be concise.If today is November 20th, then in 28 days it will be"
corr_text = "5 3 9"
num_toks_gen = 20

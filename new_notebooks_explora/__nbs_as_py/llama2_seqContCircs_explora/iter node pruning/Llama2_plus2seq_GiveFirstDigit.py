...
def generate_prompts_list(x ,y):
    ...
    for i in range(x, y):
        prompt_dict = {
            'text': f"{i} {i+2} {i+4} {i+6} "
        }
        full_ans = tokenizer.tokenize(str(i+8))[1:]  # ans_as_tokstr
        prompt_dict['text'] += full_ans[0]

        prompt_dict['corr'] = full_ans[-1]
        prompt_dict['incorr'] = tokenizer.tokenize(str(i+6))[1:][-1] # compare to only the last digit of last seqmem

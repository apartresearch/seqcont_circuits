...
i=2
corr_ans = tokenizer.tokenize('10')[1:]
f"{i} {i+2} {i+4} {i+6} " + corr_ans[0]
...
def generate_prompts_list(x ,y):
    ...
    for i in range(x, y):
        prompt_dict = {
            'text': f"{i} {i+3} {i+6} {i+9} "
        }
        full_ans = tokenizer.tokenize(str(i+12))[1:]  # ans_as_tokstr
        prompt_dict['text'] += full_ans[0]

        prompt_dict['corr'] = full_ans[-1]
        prompt_dict['incorr'] = tokenizer.tokenize(str(i+9))[1:][-1] # compare to only the last digit of last seqmem
...
prompts_by_ansLen = []
prompts_list = generate_prompts_list(10, 20)
...
mlp_scores = {}
for i in curr_circ_mlps:
    copy_circuit = curr_circ_mlps.copy()
    copy_circuit.remove(i)
    print("removed: MLP " + str(i))
    model.reset_hooks(including_permanent=True)  #must do this after running with mean ablation hook
    model = add_ablation_hook_MLP_head(model, dataset_2, curr_circ_heads, copy_circuit)
...
for lh, score in sorted_mlp_scores.items():
    print(lh, -round(circ_score-score.item(), 2))
...
with open('plus2seq_GiveFirstDigit_1prompt_b_20_scores.pkl', 'wb') as file:
    pickle.dump(all_comp_scores, file)
files.download('plus2seq_GiveFirstDigit_1prompt_b_20_scores.pkl')

...
def generate_prompts_list(x ,y):
    ...
    for i in range(x, y):
        ...
        prompt_dict['corr'] = tokenizer.tokenize(str(i+8))[1:]  # ans_as_tokstr
        prompt_dict['incorr'] = tokenizer.tokenize(str(i+6))[1:]
...
"""
Let N = ansLen
run the Nth dataset N times and sum up the logit DIFFs for the N runs.
Then take the final summed logit IDFF score for these N datasets, and
...
for ansLen, dataset in enumerate(clean_datasets):
    ...
    dataset_2 = corrupted_datasets[0] # here, we're only using double digits

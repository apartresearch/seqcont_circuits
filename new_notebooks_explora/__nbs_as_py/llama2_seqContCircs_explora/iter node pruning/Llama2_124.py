...
prompt =  "Find the next member of this sequence: 2 4 8 "
...
def generate_prompts():
    ...
    list_tokens = tokenizer.tokenize('1 2 4 ')
    for i, tok_as_str in enumerate(list_tokens):
        # if tok_as_str == '▁':
        #     prompt_dict['S'+str(i)] = ' '
        # else:
...
def generate_prompts_list_corr():
    ...
    list_tokens = tokenizer.tokenize(f"5 7 3 ")
    for i, tok_as_str in enumerate(list_tokens):
        # if tok_as_str == '▁':
        #     prompt_dict['S'+str(i)] = ' '
        # else:
...
with open('124_b_20_scores.pkl', 'wb') as file:
    pickle.dump(all_comp_scores, file)
files.download('124_b_20_scores.pkl')

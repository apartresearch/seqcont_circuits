...
num_samps_per_ptype = 1 #10 # 12 #768 512
...
def modify_strings(input_strings):
    output_strings = []
    for s in input_strings:
        # Use a regex to find and modify the numbers
        modified_string = re.sub(r'(\d+)', lambda x: str(int(x.group(0)) * 2 + 8), s)
        output_strings.append(modified_string)
    return output_strings

prompts = modify_strings(prompts)
print(prompts)
...
prompts = [pr + " 1" for pr in prompts]
prompts
...
def modify_strings(input_strings, start_number, interval):
    output_strings = []
    for s in input_strings:
        # Initialize a variable to keep track of the sequence position
        current_number = start_number - interval
        ...
        # Use a regex to find numbers and replace them with the sequence values
        modified_string = re.sub(r'\d+', replace_func, s)
        output_strings.append(modified_string)
    return output_strings

prompts = modify_strings(prompts, 10, 3)
print(prompts)
...
prompts = modify_strings(prompts, 100, 100)
print(prompts)
